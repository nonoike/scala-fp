object MainRefactored {
  ...
  def getPostalCodeResult(userId: Int): PostalCodeResult = {
    // Either型はmapやflatMapがないため、RightProjection型にして処理をする
    // EitherがLeftの場合は処理を行わない
    // .mergeで中身を畳み込んで取得する
    (for {
  ...
  def findUser(userId: Int): Either[Failure, User] = userDatabase.get(userId).toRight(UserNotFound)
  ...
  def findPostalCode(address: Address): Either[Failure, String] = address.postalCode.toRight(AddressNotHasPostalCode)
